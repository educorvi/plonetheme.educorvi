...
from plonetheme.educorvi.testing import PLONETHEME_EDUCORVI_FUNCTIONAL_TESTING
from plonetheme.educorvi.testing import PLONETHEME_EDUCORVI_INTEGRATION_TESTING
...
class ViewletIntegrationTest(unittest.TestCase):
    ...
    def test_references_viewlet_is_registered(self):
        view = BrowserView(self.portal['other-document'], self.request)
        manager_name = 'plone.abovecontenttitle'
        alsoProvides(self.request, IPlonethemeEducorviLayer)
        ...
        manager.update()
        my_viewlet = [v for v in manager.viewlets if v.__name__ == 'references-viewlet']  # NOQA: E501
        self.assertEqual(len(my_viewlet), 1)

    # XXX would be nice to have this test working:
    # def test_references_viewlet_is_not_available_on_newsitem(self):
...
class ViewletFunctionalTest(unittest.TestCase):
    ...
    layer = PLONETHEME_EDUCORVI_FUNCTIONAL_TESTING
